    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'14px 20px',borderBottom:`1px solid ${T.bdr}`,flexShrink:0}}>
        <div>
          ...
          <div style={{...mono,fontSize:10,color:T.dim}}>{visible.length} usuário{visible.length!==1?'s':''} exibido{visible.length!==1?'s':''}</div>
        </div>
        <div style={{flex:1}}/>
        <button onClick={onClose}
          style={{padding:'6px 10px',background:'transparent',border:`1px solid ${T.bdr2}`,borderRadius:6,color:T.muted,fontSize:12,cursor:'pointer'}}>
          ✕ Fechar
        </button>
      </div>

      {feedback&&(
        <div style={{margin:'10px 20px 0',padding:'8px 12px',borderRadius:6,fontSize:13,
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{padding:'10px 20px',borderBottom:`1px solid ${T.bdr}`}}>
            <input type="text" value={search} onChange={e=>setSearch(e.target.value)}
              ...
              style={{width:'100%',padding:'6px 10px',background:T.inputBg,border:`1px solid ${T.inputBdr}`,borderRadius:6,color:T.txt,fontSize:13,outline:'none'}}/>
          </div>
              ...
              <thead>
                <tr style={{position:'sticky',top:0,background:T.surface,borderBottom:`1px solid ${T.bdr}`,zIndex:2}}>
                  {['Nome','Usuário','Função','Último Acesso','Status',''].map(h=>(
                    <th key={h} style={{padding:'8px 14px',textAlign:'left',...mono,fontSize:9,color:T.dim,fontWeight:400,textTransform:'uppercase',letterSpacing:1}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return(
                    <tr key={user.id}
                      ...
                      <td style={{padding:'10px 14px'}}>
                        ...
                        <div style={{...mono,fontSize:10,color:T.dim}}>{user.email}</div>
                      </td>
                      <td style={{padding:'10px 14px',...mono,fontSize:12,color:T.muted}}>
                        {user.username}
                        {isSelf&&<span style={{marginLeft:6,fontSize:9,color:'#3b82f6',background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:3,padding:'1px 4px'}}>você</span>}
                      </td>
                      <td style={{padding:'10px 14px'}}>
                        <span style={{...mono,fontSize:10,color:role.textClr,background:`${role.clr}22`,border:`1px solid ${role.clr}44`,borderRadius:4,padding:'2px 7px'}}>{role.name}</span>
                      </td>
                      <td style={{padding:'10px 14px',...mono,fontSize:11,color:T.dim}}>{formatDateTime(user.lastLogin)}</td>
                      <td style={{padding:'10px 14px'}}>
                        <span style={{...mono,fontSize:10,borderRadius:4,padding:'2px 7px',
                        ...
                        <div style={{display:'flex',gap:6}}>
                          {canEdit&&user.isActive&&(
                            <button onClick={()=>setEditUser(user)}
                              style={{padding:'3px 10px',background:'transparent',border:`1px solid ${T.bdr2}`,borderRadius:4,color:T.muted,fontSize:11,cursor:'pointer'}}
                          ...
                          {canDeact&&user.isActive&&(
                            <button onClick={()=>setDeactConf(user)}
                              style={{padding:'3px 10px',background:'transparent',border:'1px solid #ef444455',borderRadius:4,color:theme==='light'?'#b91c1c':'#ef4444',fontSize:11,cursor:'pointer'}}
          ...
          <div onClick={e=>e.stopPropagation()}
            ...
            <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
              <button onClick={()=>setDeactConf(null)}
                style={{padding:'7px 16px',background:'transparent',border:`1px solid ${T.bdr2}`,borderRadius:6,color:T.muted,fontSize:12,cursor:'pointer'}}>Cancelar</button>
              <button onClick={()=>handleDeactivate(deactConf)}
                style={{padding:'7px 16px',background:'#ef4444',border:'none',borderRadius:6,color:'#fff',fontSize:12,fontWeight:600,cursor:'pointer'}}>
                Desativar
              </button>
            </div>
          </div>
...
function UserForm({T, theme, mono, roles, existing, currentUser, onSave, onCancel}) {
  ...
  const lbl = {...mono,fontSize:9,color:T.dim,textTransform:'uppercase',letterSpacing:1,display:'block',marginBottom:4};
  ...
  return (
    <form onSubmit={handleSubmit} style={{padding:20}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:16,color:T.txt}}>Editar Usuário</div>
      ...
      <div style={{marginBottom:12}}>
        ...
        {errors.roleId&&<div style={{fontSize:11,color:'#ef4444',marginTop:3}}>{errors.roleId}</div>}
      </div>
      ...
      <div style={{display:'flex',gap:8,marginTop:4}}>
        <button type="button" onClick={onCancel}
          style={{flex:1,padding:'8px',background:'transparent',border:`1px solid ${T.bdr2}`,borderRadius:6,color:T.muted,fontSize:12,cursor:'pointer'}}>Cancelar</button>
        <button type="submit"
          style={{flex:2,padding:'8px',background:'#3b82f6',border:'none',borderRadius:6,color:'#fff',fontSize:12,fontWeight:600,cursor:'pointer'}}>
          Salvar Alterações
        </button>
      </div>
    </form>
